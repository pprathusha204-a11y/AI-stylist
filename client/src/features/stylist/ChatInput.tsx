  ...
  return (
    <div className="fixed bottom-3 left-1/2 z-20 w-[calc(100%-1.5rem)] max-w-3xl -translate-x-1/2 sm:bottom-4 sm:w-[calc(100%-2rem)]">
      ...
      <form onSubmit={handleSubmit} className="flex items-center rounded-full border border-slate-200 bg-white px-2 py-2 shadow-lg sm:px-3">
        ...
        <button type="button" disabled={disabled || reading} onClick={() => fileInput.current?.click()} className="flex h-9 w-9 shrink-0 items-center justify-center text-slate-600 transition hover:text-slate-900 disabled:opacity-50" aria-label="Upload image" title="Upload image"><ImagePlus size={16} strokeWidth={1.6} /></button>
